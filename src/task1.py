...
def find_household(user_id):
    called_with(user_id)
    return household_db.get(QUERY.user_ids.any(user_id))
...
def create_new_order(user_id):
    ...
    user_household = find_household(user_id)
    if not user_household:
        log_error(f"can't find household with user_id {user_id}")
        return False

    household_id = user_household["household_id"]
