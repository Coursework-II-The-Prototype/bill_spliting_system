...
order.insert({'type': 'order', 'user_id': 1, 'item_id': 1, 'amount': 1})
order.insert({'type': 'order', 'user_id': 2, 'item_id': 2, 'amount': 2})
...
def create_new_order(user_id):
    ...
    new_order = {
        'type': 'order',
        'order_id': max(order.all(), 
                        key=lambda x: x['order_id'])['order_id'] + 1,
        'users': user_ids,
